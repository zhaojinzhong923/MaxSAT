...
bool HistLS::parse_parameters2(int argc, char **argv)
{
    int i = 0;
    int temp_para = 0;
    // cout << "c test:" << endl;
...
void HistLS::settings()
{
    ...
    if (1 == problem_weighted) // Weighted
    {
        if (0 != num_hclauses) // partial
        {
            ...
            coe_tuned_weight = 1.0/(double)floorToPowerOfTen(double(total_soft_weight) / (double)(num_sclauses));
        ...
        else // not paritial
        {
            ...
            coe_tuned_weight = ((double)coe_soft_clause_weight)/floorToPowerOfTen((double(total_soft_weight) / (double)(num_sclauses)));
...
void HistLS::build_neighbor_relation()
{
    int i, j, count;
    int v, c, n;
...
void HistLS::build_instance(char *filename)
{
    total_soft_length = 0;
    total_hard_length = 0;
    // max_soft_weight = 1;
